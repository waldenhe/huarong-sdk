...
public class HttpRequestSimple {
    ...
    public Object postSendHttp(String url, Object inputObj) {
        ...
        try {
            ...
            if (ret == HttpStatus.SC_OK) {
                ...
                logger.info("请求[" + url + "]消耗时间 " + (end - start) + "毫秒");
                return returnObj;
            }
            return null;
    ...
    public String postPramaList(String url, BasicNameValuePair[] list) {
        List<BasicNameValuePair> nvList = new ArrayList<BasicNameValuePair>();
        for (BasicNameValuePair nameValue : list) {
            nvList.add(nameValue);
        }
        return postPramaList(nvList, url);
    }

    public String postPramaList(List<BasicNameValuePair> list, String url) {
        HttpClient httpClient = CustomHttpClient.GetHttpClient();
        HttpPost post = new HttpPost(url);
